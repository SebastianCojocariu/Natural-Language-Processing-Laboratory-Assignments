...
for i in range(len(sentences[:N])):
	...
	for i in range(len(sentence_words)):
		if sentence_words[i] not in visited:
			visited[sentence_words[i]] = True
			if simple_lemmas[i] != lemmas_with_pos[i]:
				print("{}|{}|{}|{}".format(words_tags[i][0], words_tags[i][1], simple_lemmas[i], lemmas_with_pos[i]))
print()
...
grammar = nltk.CFG.fromstring("""
	...
	V -> "eat" | "like" | "go" | "make"	| "do"
	Pronoun -> "I" | "you" | "You" | "she" | "She" | "he" | "He" | "we" | "We" | "they" | "They" | "it" | "It"
	TO -> "to"
	A -> "beautiful" | "clean"
	Det -> "the" | "The" | "a" | "A" | "an" | "An" 
	Prep -> "at" | "in" | "on" | "with" | "to"
	N -> "pie" | "ice-cream" | "spoon" | "soup" | "silver" | "apples"
	""")

for tree in nltk.RecursiveDescentParser(grammar).parse("I like to eat the soup with a clean spoon".split()):
	a = tree
for tree in nltk.RecursiveDescentParser(grammar).parse("I like to eat the soup with a spoon".split()):
	b = tree
...
sentence_to_words2 = "I like apples".split()
